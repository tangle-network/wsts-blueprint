    ...
    M: Mpc<ProtocolMessage = Msg>,
{
    ...
    let public_key_frost_format = verifying_key
        .serialize()
        .map_err(|e| KeygenError::MpcError(format!("Failed to serialize VerifyingKey: {e}")))?;
    state.public_key_frost_format = public_key_frost_format;
    state.party = Arc::new(parking_lot::Mutex::new(Some(party)));

    tracing::info!("Keygen finished computing secret");
